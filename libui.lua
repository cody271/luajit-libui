...
local Control = pl.class(pl.class.properties)
...
local Checkbox = pl.class(Control)
...
local ColorButton = pl.class(Control)
...
local Combobox = pl.class(Control)
...
local DatePicker = pl.class(Control)
...
local DateTimePicker = pl.class(Control)
...
local EditableCombobox = pl.class(Control)
...
local FontButton = pl.class(Control)
...
function Grid:InsertAt(c, existing, at, xspan, yspan, hexpand, halign, vexpand, valign)
  _lib.uiGridInsertAt(self._raw, ffi.cast("uiControl*", c._raw), ffi.cast("uiControl*", existing._raw), at, xspan, yspan, hexpand, halign, vexpand, valign)
end
...
function Grid:Append(c, left, top, xspan, yspan, hexpand, halign, vexpand, valign)
  _lib.uiGridAppend(self._raw, ffi.cast("uiControl*", c._raw), left, top, xspan, yspan, hexpand, halign, vexpand, valign)
end

Grid._name = "Grid"
M.Grid = Grid

local Group = pl.class(Control)
...
local HorizontalBox = pl.class(Box)
...
local HorizontalSeparator = pl.class(Control)
...
local MultilineEntry = pl.class(Control)
...
local NonWrappingMultilineEntry = pl.class(Control)
...
local PasswordEntry = pl.class(Control)
...
local ProgressBar = pl.class(Control)
...
local RadioButtons = pl.class(Control)
...
local SearchEntry = pl.class(Control)
...
local Spinbox = pl.class(Control)
...
local TimePicker = pl.class(Control)
...
local VerticalBox = pl.class(Box)
...
local VerticalSeparator = pl.class(Control)
...
function M.OnShouldQuit(f, data)
